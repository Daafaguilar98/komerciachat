<template>
  <div id="komerciaChat">
    <div class="komercia_chat" v-show="openchat">
      <div class="komercia_chat_header" style="background-color: var(--color_principal);">
      ...
      <template v-if="userData.id">
        <div class="komercia_chat_messages" id="KomerciaMessages">
          <p v-for="(item, index) in messages" :key="index" :class="item.from" style="background-color: var(--color_principal);">{{ item.message }}</p>
        </div>
        <div class="komercia_chat_input">
          <input v-model="message" placeholder="Escribir una respuesta">
...
<script>
import Login from './KomerciaChat/Login.vue'
...
export default {
  ...
  methods: {
    constructor() {
      ...
      loadMessages(){
        var chats = firebase.database().ref('chats/').orderByChild('store_id').equalTo(parseInt(this.storeData.id_tienda))
        chats.on('value', (snapshot) => {
          ...
          if (snapshot.val()) {
            ...
            this.key = chatsTMP.filter(chat => chat.customer_id == this.userData.id)[0].key
          }
          if (this.key) {
            var chatsMessages = firebase.database().ref(`chats/${this.key}/messages`)
